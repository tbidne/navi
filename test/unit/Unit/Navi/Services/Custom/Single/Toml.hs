...
testCommandResultSuccess0 = assertDecode @SingleToml desc txt $ \toml -> do
  case toml ^. #parser of
    Nothing -> pure ()
...
testCommandResultSuccess1 = testCase desc $ do
  ...
  toml1 <- assertRight $ decode @SingleToml cfg1
  ...
  toml2 <- assertRight $ decode @SingleToml cfg2
...
testCommandResultSuccess2 = testProp "testCommandResultSuccess2" desc $ do
  ...
  parser <- case decode @SingleToml cfg of
    Left err -> do
